...
def get_request(url, proxies):
    ...
    while True:
        ...
        try:
            logger.debug(f"Using proxy {pp}")
            r = requests.get(url, proxies=pp)
            return r
        except Exception as e:
            logger.warn(f'{str(e)}Connect to {p} error. Skipping...')
...
def write_news_to_csv(news: List[NewsApiPayload], path='.\\result'):
    dict = {
    ...
    res_path = os.path.join(path, 'news')
    if not os.path.exists(res_path):
        os.makedirs(res_path)
    df.to_csv(os.path.join(res_path, 'news.csv'))


def write_finance_to_csv(finances: List[FinanceYahooResponse], path='.\\result'):
    dict = {
    ...
    res_path = os.path.join(path, 'finance')
    if not os.path.exists(res_path):
        os.makedirs(res_path)
    df.to_csv(os.path.join(res_path, 'finances.csv'))


def do_task(custom_path,
            from_date=(datetime.today() - timedelta(30)).strftime('%Y-%m-%d'),
            to_date=datetime.today().strftime('%Y-%m-%d'),
            key_word='bitcoin',
            sources=", ".join(['techcrunch']),
            companies=('AAPL', 'MSFT', 'NFLX', 'AMZN')
            ):
    logger.info("Parsing script started")
    proxies = get_proxies()
    params = SearchParams(key_word,
                          sources,
                          from_date,
                          to_date,
                          'publishedAt',
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_url):
            try:
                res = future.result()
                logger.debug("Got a result from future")
                results_dict[type(res[0])] = res
            except Exception as exc:
                logger.error('generated an exception: %s' % exc)

    try:
        write_news_to_csv(results_dict[NewsApiPayload], custom_path)
        write_finance_to_csv(results_dict[FinanceYahooResponse], custom_path)
    except Exception as e:
        logger.error(f'Exception occurred: {str(e)}')
    logger.info("Parse script finished")
